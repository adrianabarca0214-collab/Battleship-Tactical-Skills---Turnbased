...
export const SHIPS_CONFIG_DEFAULT: Omit<Ship, 'positions' | 'isSunk' | 'isDamaged' | 'type'>[] = [
...
export const SHIPS_CONFIG_LARGE: Omit<Ship, 'positions' | 'isSunk' | 'isDamaged' | 'type'>[] = [
  { name: 'Carrier', length: 5 },
  { name: 'Battleship', length: 4 },
  { name: 'Cruiser', length: 3 },
  { name: 'Submarine', length: 3 },
...
export const SHIPS_CONFIG_TACTICAL: Omit<Ship, 'positions' | 'isSunk' | 'isDamaged'>[] = [
...
export const getGameConfig = (playerCount: number, mode: 'CLASSIC' | 'SCORE_ATTACK' | 'TACTICAL') => {
    ...
    return {
        gridDimensions: { rows: 10, cols: 15 },
